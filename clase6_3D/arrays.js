...
//Funcion map, devuelve el Array original con lo que yo haya agregado en el callback, no modifica la original
const arrayNumeros = [1,2,3];

const duplicados = arrayNumeros.map((elemento)=>{
    return elemento*2;
});
...
const hola = razas.map((elemento)=>{
    return "Hola";
});

console.log(hola);

//Funcion filter, crea un nuevo array a partir de otro segun la condicion que pongamos, el original queda intacto
const pares = arrayNumeros.filter((e)=>{
    if(e % 2 === 0)
    {
        return true;
    }
...
//otra forma de hacerlo: vec.filter(e => !(e % 2));

console.log(pares);

//Funcion reduce va iterando sumando el elemento actual con el anterior, si le pongo cero va a hacer que la primer iteracion tome como actual el index 0, en este caso daria lo mismo
const total = arrayNumeros.reduce((prev, actual)=>{
    return prev + actual;
}, 0);
